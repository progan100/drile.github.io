...
//2)Функция открытия модального окна обратной связи
function openModal() {
...
//4)Слушаем события для открытия модального окна при нажатии на кпопку
openModalButton.addEventListener('click', openModal);

//5)Слушаем события для закрытия модального окна при нажатии на крестик
closeButton.addEventListener('click', closeModal);


//Отправка формы модального окна, и модальное окно с успешной отправкой формы
let form = document.getElementById('form');
let submitButton = document.getElementById('submitButton');
let succes = document.getElementById('succes');
form.addEventListener('submit', function (event) {
    event.preventDefault();
    succes.classList.add('animate__fadeIn');
...
let loginModalForm = document.getElementById('login-modal-form');
...
menuItems.forEach(function (item) {//item-объект
...
function addToCartHandler() {
    //увеличение значения счетчика на один
    itemCount++;
...
products.forEach(function (product) {
    ...
    //Получаем цену товара, и преобразовываем ее в число
    ...
    addToCartBtn.addEventListener('click', function () {
        ...
        if (existingCartItem) {
            existingCartItem.quantity++;//кол-во на один увеличиваем
        } else {
            let newItem = {
                name: productName,
                price: productPrice,
